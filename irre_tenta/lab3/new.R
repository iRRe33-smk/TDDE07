...
hist(exp(draws_posterior),add=T, freq=F, breaks = 100, xlim=range(0,80),col="blue")
legend("topright",legend=c("historical data", "posterior draws"), col=c("red", "blue"), lty=1)
...
hist(poisson_draws, main = "Predictive distribution", sub =paste("glm predicts: ",exp(model$coefficients%*%sample)), freq=F)
...
plot(path[,2],type="line",col="blue", main="trajectories sampled with our two sets of parameters")  
